...
class Road:
    ...
    def create_roads(self):
        ...
        exit_segment = self.add_curve(40, 20, 5, 30, 5)
        exit_segment = self.add_curve(-30, exit_segment, 4, 20, 10)
        exit_segment = self.add_curve(20, exit_segment, 4, 20, 10)
        exit_segment = self.add_curve(5, exit_segment, 10, 20, 10)

    def add_curve(self, curve, start, enter, hold, exit):
        def ease_in(a, b, per):
        ...
        for i in range(0, enter):
            self.segments[start + i].curve = ease_in(0, curve, i / enter)

        start += i
        for i in range(0, hold):
            self.segments[start + i].curve = curve

        start += i
        for i in range(0, exit):
            self.segments[start + i].curve = ease_out(curve, 0, i / exit)
