...
class Road:
    ...
    def create_roads(self):
        ...
        exit_segment = self.add_ease("curve", -10, exit_segment, 4, 20, 10)
...
class Game(arcade.Window):
    ...
    def on_draw(self):
        ...
        current = self.camera.current_segment
        current = self.road.get_segment_index(self.player.pos.z)
        current = self.camera.road.segments[current]
        position, scale = project3d(
            Vertex(self.player.pos.x, -current.world.y, self.player.pos.z - self.tmp),
            self.camera,
            0, 0,
            1,
