...
class Road:
    ...
    def create_roads(self):
        ...
        if 0:
            exit_segment = self.add_ease("curve", 40, 20, 5, 30, 5)
            exit_segment = self.add_ease("curve", -30, exit_segment, 4, 20, 10)
            exit_segment = self.add_ease("curve", 20, exit_segment, 4, 20, 10)
            exit_segment = self.add_ease("curve", 5, exit_segment, 10, 20, 10)

        exit_segment = self.add_ease("hill", -400, 20, 5, 100, 10)
    ...
    def project3d(self, point, camera, offset_x, offset_z):
        # translate world coordinates to camera coordinates
        translated_camera = Vertex(
            point.world.x - camera.pos.x + offset_x,
    ...
    def render3d(self, camera):
        ...
        for i in range(len(self.segments)):
            ...
            offset_z = self.road_length if segment_i < camera_segment.index else 1

            # todo: can be optimized to store the projected coordinates per update instead
            #       of computing each time
            p1 = self.project3d(current_segment, camera, offset_x, offset_z)
            p2 = self.project3d(next_segment, camera, offset_x + dx, offset_z)
            ...
            print(segment_i, current_segment.world.y, p1.y, next_segment.world.y, p2.y)
            if p2.y < current_clip: continue

            print(current_clip, p2.y)
...
class Game(arcade.Window):
    ...
    def on_draw(self):
        ...
        translated_camera = Vertex(
            self.player.x - self.camera.pos.x,
            self.player.y - self.camera.pos.y,
            self.player.z - self.camera.pos.z,
        )

        self.sprite_car.scale = (self.camera.near_plane / translated_camera.z)
