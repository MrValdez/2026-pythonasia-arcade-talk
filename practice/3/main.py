...
class Player:
    ...
    def update(self, dt):
        ...
        if self.camera.current_segment:
            self.pos.x += (speed * self.camera.current_segment.curve * self.car_drift)
...
class Road:
    ...
    def create_roads(self):
        ...
        exit_segment = self.add_ease("curve", 20, 20, 5, 30, 5)
        ...
        exit_segment = self.add_ease("hill", 300, 20, 100, 10, 1)
        exit_segment = self.add_ease("hill", -200, exit_segment + 40, 5, 20, 10)
    ...
    def render3d(self, camera):
        ...
        start_segment_i = self.get_segment_index(camera.pos.z)
        start_segment = self.segments[start_segment_i]

        offset_x = 0
        dx = start_segment.curve * self.get_segment_per(camera.pos.z)
        drawn = 0

        current_clip = 0
        camera_segment = start_segment
...
class Game(arcade.Window):
    ...
    def on_draw(self):
        ...
        start_segment_i = self.road.get_segment_index(self.camera.pos.z)
        camera_segment = self.road.segments[start_segment_i]

        for i, (bg, depth, offset) in enumerate(self.backgrounds):
            if camera_segment and depth:
                self.backgrounds[i][2][0] += (1 / depth) * camera_segment.curve
            arcade.draw_texture_rect(bg, arcade.LBWH(offset[0], offset[1], window_width, window_height))

        #return
        #self.road.render2d()
        self.road.render3d(self.camera)

        #scale =(self.camera.near_plane / translated_camera.z)
        #scale =(self.camera.near_plane / (self.player.pos.z or 1))
        #scale =(self.camera.near_plane / (self.camera.dist_to_player + translated_camera.z))

        current = self.road.get_segment_index(self.player.pos.z)
        current = self.road.segments[current]
        ...
        scaling_constant = 1500     # number to tweak. haven't found a mathematical way to extract
        self.sprite_car.scale = 1/ (1200 * -scale)
